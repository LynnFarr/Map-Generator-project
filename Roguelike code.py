...
///    the dungeon isnt single connected.
/// 5. The mazes will have a lot of dead ends. Finally, we remove those by
///    repeatedly filling in any open tile thats closed on three sides. When
...
abstract class Dungeon extends StageBuilder {
  ...
  void _addRooms() {
    for (var i = 0; i < numRoomTries; i++) {
      ...
      // TODO: This isnt very flexible or tunable. Do something better here.
  ...
  void _connectRegions() {
    ...
    // Keep connecting regions until we are down to one.
    while (openRegions.length > 1) {
      ...
      // Merge the connected regions. We will pick one region (arbitrarily) and
      ...
      // some of the ones we are merging now.
      for (var i = 0; i <= _currentRegion; i++) {
      ...
      // Remove any connectors that are not needed anymore.
      connectors.removeWhere((pos) {
        // Do not allow connectors right next to each other.
        if (connector - pos < 2) return true;

        // If the connector no long spans different regions, we do not need it.
        var regions = connectorRegions[pos].map((region) => merged[region])
            .toSet();

        if (regions.length > 1) return false;

        // This connecter is not needed, but connect it occasionally so that the
        // dungeon is not singly-connected.
